...
class ConversationAnalyzer:
    ...
    async def _get_similar_conversations(
        self, app_id: str, tags: List[str], limit: int = 10
    ) -> List[Dict[str, Any]]:
        ...
        result = await self.carrot_quest.get_app_conversations(
            app_id=app_id, tags=tags, limit=limit
        )
        conversations = cast(List[Dict[str, Any]], result.get("conversations", []))
    ...
    async def analyze_conversation(
        ...
        tags = cast(List[str], conversation.get("tags", []))
        ...
        pattern_content = {
            ...
            "similar_patterns": [self._extract_pattern(conv) for conv in similar],
        }

        pattern_hash = self._generate_pattern_hash(pattern_content)

        # Generate assistant configuration
        assistant_config = self._generate_assistant_config(pattern_content, similar)

        # Determine optimal MCP tools
        mcp_tools = self._select_mcp_tools(pattern_content, assistant_config)
    ...
    def _generate_assistant_config(
        self, pattern: Dict[str, Any], similar: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        config = {
            ...
            "tools": list([{"type": "code_interpreter"}]),  # Default tools
            "metadata": {
        ...
        if similar:
            successful_patterns = [
                conv for conv in similar if conv.get("resolved", False)
            ]
            if successful_patterns:
                config["metadata"]["similar_patterns_count"] = len(successful_patterns)
                avg_resolution_time = sum(
                    conv.get("resolution_time", 0) for conv in successful_patterns
    ...
    def _select_mcp_tools(
        self, pattern: Dict[str, Any], assistant_config: Dict[str, Any]
    ) -> List[str]:
        ...
        tools: List[str] = base_tools.copy()
        ...
        if message_type == "support_request":
            tools.extend(["get_user", "set_user_props", "record_user_event"])
        ...
        if "technical" in tags:
            tools.extend(["get_app_users", "get_active_users"])
